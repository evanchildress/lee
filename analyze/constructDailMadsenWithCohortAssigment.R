...
#Cohort assignment
    ...
    #overall mean length for each cohort in each year
        muSize[g,t]~dnorm(muPriors[g,1],0.001)I(muPriors[g,2],muPriors[g,3])
    #fixed year effect on pu (inclusion in group g)
        betaSize[g,t]~dgamma(1,1)
        pu[g,t]<-betaSize[g,t]/sum(betaSize[,t]) #pu[,t] must sum to 1
      }
...
#           }
#             counts[s,t,a,p]<-cut(sum(thisEverything[s,t,a,p,]))
#         }
#       }
#     }
#   }
